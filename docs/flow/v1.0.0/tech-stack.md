...
| テスト (unit)  | bun test（バックエンド・packages） + Vitest（Web）  | Bun テストランナーが速い。Web は Vite と統合                                                                                                                   |
| テスト (E2E)   | bun test + Hono `app.request()` + 実 Postgres       | Hono アプリを実プロセス起動せず、migration 適用済みの実 DB に対して結合動作を担保。CI は GitHub Actions の `services.postgres`、ローカルは Supabase CLI を使う |
